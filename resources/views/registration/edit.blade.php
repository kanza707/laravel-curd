...
<body>
  ...
  <div class="container mt-5">
    ...
    @endif
        <form method="POST" action="{{ route('registration.update',$ruser->id) }}">
          @csrf
          @method('PATCH')
          <div class="row mb-3">
        ...
        @enderror" id="inputname" name="name" value="{{ $ruser->name }}">
              @error('name')
        ...
        @enderror
          ...
          @enderror" id="inputEmail3" name="email" value="{{ $ruser->email }}">
                @error('email')
          <div class="invalid-feedback">
            {{ $message }}
          </div>
